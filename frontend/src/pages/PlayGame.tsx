...
const PlayGame = () => {
  ...
  const { loading, data } = useQuery<GetGameQuery>(GetGame, {
    variables: { id },
  });
  ...
  const [submitAnswer, { loading: mutationLoading }] =
    useMutation<SubmitAnswerMutation>(SubmitAnswer);
  const [questions, setQuestions] = useState<
    Pick<Question, "id" | "question" | "answers">[]
  >([]);

  const [endGame] = useMutation<EndGameMutation>(EndGame, {
    variables: { gameId: id },
  });
  ...
  useInterval(() => {
    if (question) {
      if (timeRemaining === 0) {
        ...
        if (!q) {
          endGame();
          navigate(`/game/${id}/finish/${player}`);
          return;
        } else {
          setTimeRemaining(30);
          setQuestion(q);
  ...
  }, [submitAnswer, id, question, answer, timeRemaining, player]);
